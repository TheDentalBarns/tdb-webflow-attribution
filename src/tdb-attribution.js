(() => {
  ...
  function isSameSite(url) {
    return Boolean(
      url && normalizeHost(url.hostname) === normalizeHost(window.location.hostname)
    );
  }
  ...
  function extractCampaign(url) {
    ...
    return {
      ...
      utm_term: utmTerm,
      meta_campaign_id:
        getParameter(params, 'meta_campaign_id', 'campaign_id') ||
        utmId ||
        (looksLikePlatformId(utmCampaign) ? utmCampaign : ''),
      meta_adset_id:
        getParameter(params, 'meta_adset_id', 'adset_id') ||
        (looksLikePlatformId(utmTerm) ? utmTerm : ''),
      meta_ad_id:
        getParameter(params, 'meta_ad_id', 'ad_id') ||
        (looksLikePlatformId(utmContent) ? utmContent : ''),
  ...
  function classifyTouch(campaign, referrer) {
    ...
    const paidMedium = /^(paid|paid_social|social_paid|cpc|ppc|paid_search|display|retargeting)$/.test(
      medium
    );
    const socialMedium = /social/.test(medium);
    const organicSearchHost = /(^|\.)(google|bing|yahoo|duckduckgo|ecosia)\./i.test(
      referrerHost
    );
    const socialReferrerHost = /(instagram|facebook|tiktok|linkedin|pinterest|youtube)\./i.test(
      referrerHost
    );

    if (hasMeta || campaign?.ttclid || campaign?.li_fat_id) {
      return { channel: 'paid_social', platform: platform || 'social' };
    }

    if (hasGoogleAds || campaign?.msclkid || medium === 'paid_search') {
      return { channel: 'paid_search', platform: platform || 'search' };
    }

    if (paidMedium && (socialMedium || /instagram|facebook|meta|tiktok|linkedin|pinterest/.test(source))) {
      return { channel: 'paid_social', platform: platform || 'social' };
    }

    if (paidMedium) {
      return { channel: medium === 'display' ? 'display' : 'paid_search', platform };
    }

    if (/email|newsletter/.test(medium) || source === 'email') {
      return { channel: 'email', platform: platform || 'email' };
    }

    if (medium === 'organic' || organicSearchHost) {
      return { channel: 'organic_search', platform: platform || 'search' };
    }

    if (socialMedium || socialReferrerHost) {
      return { channel: 'organic_social', platform: platform || 'social' };
    }

    if (source || medium) {
      return { channel: medium || 'campaign', platform };
    }

    if (referrer && !isSameSite(referrer)) {
      return { channel: 'referral', platform: platform || normalizeHost(referrer.hostname) };
    }
  ...
  function migrateLegacyAttribution(legacy) {
    ...
    const campaign = {
      ...
      utm_id: legacy.utm_id || '',
      utm_content: legacy.utm_content || '',
      utm_term: legacy.utm_term || '',
      meta_campaign_id: legacy.meta_campaign_id || '',
      meta_adset_id: legacy.meta_adset_id || '',
      meta_ad_id: legacy.meta_ad_id || '',
      meta_placement: legacy.meta_placement || '',
      fbclid: legacy.fbclid || '',
      gclid: legacy.gclid || '',
      gbraid: legacy.gbraid || '',
      wbraid: legacy.wbraid || '',
      msclkid: legacy.msclkid || '',
      ttclid: legacy.ttclid || '',
      li_fat_id: legacy.li_fat_id || ''
    };
    const legacyReferrer = safeUrl(legacy.initial_referrer || '');
    const classification = classifyTouch(campaign, legacyReferrer);
    const touch = {
      path: normalizePath(legacy.first_landing_path || '/'),
      url: legacy.first_landing_url || '',
      at: legacy.first_touch_at || new Date(pageLoadedAt).toISOString(),
      method: 'legacy_migration',
      initial_referrer: legacy.initial_referrer || '',
      channel: classification.channel,
      platform: classification.platform,
      ...campaign
    };
    ...
    return {
      schema_version: 2,
      attribution_id: legacy.attribution_id,
      first_touch: touch,
      last_touch: touch,
      visit_number: Number(legacy.visit_number) || 1,
      last_seen_at: Number(legacy.last_seen_at) || pageLoadedAt
    };
  }
  ...
  function chooseIncomingTouch() {
    ...
    if (hasCampaignEvidence(currentCampaign)) {
      return buildTouch(currentUrl, 'current_url');
    }

    if (referrerUrl && isSameSite(referrerUrl)) {
      const referrerCampaign = extractCampaign(referrerUrl);

      if (hasCampaignEvidence(referrerCampaign)) {
        return buildTouch(referrerUrl, 'internal_referrer_recovery');
      }
    }

    if (referrerUrl && !isSameSite(referrerUrl)) {
      return buildTouch(currentUrl, 'external_referrer', referrerUrl);
    }
  ...
  function isMeaningfulNonDirectTouch(touch) {
    return Boolean(
      touch &&
        (hasCampaignEvidence(touch) ||
          touch.channel === 'referral' ||
          touch.channel === 'organic_search' ||
          touch.channel === 'organic_social' ||
          touch.channel === 'email')
    );
  }

  function addJourneyPath(session, path) {
    const normalized = normalizePath(path);
    const paths = Array.isArray(session.journey_paths) ? session.journey_paths : [];

    if (paths[paths.length - 1] === normalized) return;

    session.page_count = (Number(session.page_count) || 0) + 1;

    if (paths.length >= MAX_JOURNEY_STEPS) {
      paths.splice(1, 1);
    }

    paths.push(normalized);
    session.journey_paths = paths;
  }

  function createSession(incomingTouch) {
    const session = {
      session_started_at: new Date(pageLoadedAt).toISOString(),
      session_landing_path: incomingTouch.path || currentPath,
      journey_paths: [],
      page_count: 0,
      last_seen_at: pageLoadedAt
    };

    if (incomingTouch.method === 'internal_referrer_recovery') {
      addJourneyPath(session, incomingTouch.path);
    }
  ...
  if (!persistent) {
    ...
    if (timedOut) {
      persistent.visit_number = (Number(persistent.visit_number) || 1) + 1;
      session = null;
    }

    if (isMeaningfulNonDirectTouch(incomingTouch)) {
      persistent.last_touch = incomingTouch;
    }
  }
  ...
  if (!session || timedOut) {
    ...
    if (incomingTouch.method === 'internal_referrer_recovery' && !session.journey_paths?.length) {
      addJourneyPath(session, incomingTouch.path);
    }
  ...
  function bindForm(form) {
    ...
    form.addEventListener(
      ...
      () => {
        saveState();
        populateForm(form);
        setHiddenField(form, 'tdb_submission_path', currentPath);
  ...
  function activateStorage() {
    ...
    if (storedSession?.session_landing_path) {
  ...
  if (CONFIG.requireConsent) {
    window.addEventListener('CookieScriptLoaded', activateStorage);
    window.addEventListener('CookieScriptAccept', activateStorage);
    window.addEventListener('CookieScriptAcceptAll', activateStorage);
    window.addEventListener(`CookieScriptCategory-${CONSENT_CATEGORY}`, activateStorage);
  }

  window.TDBAttribution = Object.freeze({
    version: VERSION,
    refresh: refreshAllForms,
    status: () => JSON.parse(JSON.stringify({ persistent, session, values: formValues() }))
  });
})();
